<template>
    <div>
        <div class="toolbar">
            ...
            >
                <div v-for="(item, index) in canvasSizeList" @click="changeCanvasSize(item)">
                    {{ item[0] }}*{{ item[1] }}
                </div>
            </div>
            <span
                v-for="(item, index) in preSizeList"
                @click="changeCanvasSizeSelect(item, index)"
                :style="{ backgroundColor: activeSize === index ? 'red' : '' }"
            >
                {{ item.title }}
            </span>
            <div class="canvas-config">
...
export default {
    ...
    methods: {
        ...
        changeCanvasSizeSelect(item, index) {
            this.activeSize = index;
            this.canvasSizeList = item.size;
            this.canvasStyleData.width = item.size[0][0];
            this.canvasStyleData.height = item.size[0][1];
        },
