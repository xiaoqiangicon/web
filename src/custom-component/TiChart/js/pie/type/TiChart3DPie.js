...
class TiChartBasePie extends TiAbstractChartType {
    ...
    createChart(isFirst) {
        ...
        // });
        let dataArr = this.option.chartData[1] ? this.option.chartData[1] : [50, 40, 30, 20, 10];
        ...
        let total = dataArr.reduce((a, b) => {
            return a + b;
        }, 0);
        let maxLength = String(Math.max(...dataArr)).length;
        let percentLength = String(Math.max(...dataArr.map((item) => ((item / total) * 100).toFixed(2)))).length;
        ...
        function getParametricEquation(startRatio, endRatio, isSelected, isHovered, k, h) {
            ...
            return {
                ...
                z: function (u, v) {
                    ...
                    if (u > Math.PI * 2.5) {
                        return Math.sin(u) > 0 ? (Math.sin(u) * h * 0.1 * 0.1) / Math.pow(10, count) : 0;
                    }
                    // return Math.sin(v) > 0 ? 1 * h * .1 : -1;
                    // return Math.sin(v) > 0 ? 1 *0.6* h * .1 : 0
                    return Math.sin(v) > 0 ? (1 * h * 0.1 * 0.7 * 1.8) / (num / 20) / Math.pow(10, count) : 0;
                },
            };
        }
            ...
            let option = {
                ...
                legend: {
                    ...
                    formatter: (name) => {
                        ...
                        if (String(obj.pieData.value).indexOf(".") === -1) {
                            arr.pop();
                        }
                    ...
                    formatter: (params) => {
                        if (params.seriesName !== "mouseoutSeries") {
                            return `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:${
                                params.color
                            };"></span>${params.seriesName}\t${option.series[params.seriesIndex].pieData.value}万元`;
                        }
                    },
                },
                xAxis3D: {
        ...
        let pa;
        if (this.option.chartData[0]) {
            pa = this.option.chartData[1].map((item, index) => {
                return {
                    value: item,
                    name: this.option.chartData[0][index],
                };
            });
        }
        else {
          pa=[{ value: 50 }, { value: 40 }, { value: 30 }, { value: 20 }, { value: 10 }]
        }
        let option = getPie3D(pa, 0.0);
