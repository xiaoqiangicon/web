...
class TiChartRosePie extends TiAbstractChartType {
    ...
    createChart(isFirst) {
        ...
        this.initChart(options, "getRosePieOption", isFirst);
    }
    /**
     * @description 改变图表数据
        ...
        let series = [
            {
                ...
                radius: this.isUndefined(style.radius)
                    ? [style.radius[0] + "%", style.radius[1] + "%"]
    ...
    changeChartStyle(style) {
        let convertStyle = this.convertStyleData(style, "getRosePieOption");
